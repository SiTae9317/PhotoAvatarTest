...
public class MeshIndexResorting2 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = 0; i < faceLineIndexs.Count; i++)
        {
            ...
            Vector3 keepVecPos = verties[faceLineIndexs[i]];
            keepVecPos.z = 0.0f;

            dynamicPoints.Add(keepVecPos * adderVal);
        }
    ...
    public Vector3 calcBarycentValue(int[] detectIndexs, Vector3 targetPoint)
    {
        Vector3 r = Vector3.zero;

        Vector2 p0 = new Vector2(dynamicPoints[detectIndexs[0]].x, dynamicPoints[detectIndexs[0]].y);
        Vector2 p1 = new Vector2(dynamicPoints[detectIndexs[1]].x, dynamicPoints[detectIndexs[1]].y);
        Vector2 p2 = new Vector2(dynamicPoints[detectIndexs[2]].x, dynamicPoints[detectIndexs[2]].y);
    ...
    public int[] isInner(Vector3 targetPoint)
    {
        for (int i = 0; i < guideTriangles.Count; i += 3)
        {
            ...
            else if (CustomRayCast.triangleRayCast(p0, p1, p2, pp, Vector3.forward, ref hitpos))
            {
                int[] returnVal = new int[3];
                returnVal[0] = index0;
                returnVal[1] = index1;
