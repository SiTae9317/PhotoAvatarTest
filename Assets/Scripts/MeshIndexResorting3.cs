...
public class MeshIndexResorting3 : MonoBehaviour
{
    ...
    public void genWire(List<Vector3> detectFacePoints)
    {
        ...
        //    if (numbers.Length > 1)
        //    {
        //        faceLineIndexs.Add(int.Parse(numbers[numbers.Length - 1]));
        //    }
        //}
        ...
        for (int i = 0; i < 68; i++)
        {
            ...
            if (i < 17)
            {
                ...
                if (i > 3 && i < 13)
                {
                    float addOffset = (1.0f / ((float)(Mathf.Abs(i - 8)) + 1.0f)) * 0.15f;
                    if (i == 6 || i == 10)
                    {
                        ;
                    }
                    ...
                    {
                        addOffset = Mathf.Min(addOffset, 0.025f);
                    }
                    adderVal += addOffset;
                }
            }
            dynamicPoints.Add(detectFacePoints[i] * adderVal);
        }
        ...
        faceWidth = Vector3.Distance(p0, p16);
        faceHeight = Vector3.Distance(p8, p100);
    ...
    void wireCopyTest()
    {
        MeshIndexResorting2 mr2 = go.GetComponent<MeshIndexResorting2>();
        Matrix4x4 ltw = go.transform.localToWorldMatrix;
        MeshFilter mf = go.GetComponent<MeshFilter>();
        Mesh mesh = mf.mesh;

        List<Vector3> mv = new List<Vector3>();
        List<Vector3> mn = new List<Vector3>();
        List<int> mt = new List<int>();
        List<Vector2> muv = new List<Vector2>();

        mv.AddRange(mesh.vertices);
        mn.AddRange(mesh.normals);
        mt.AddRange(mesh.triangles);
        muv.AddRange(mesh.uv);
        ...
        {
            Vector3 curVPos = ltw.MultiplyPoint3x4(mv[i]);

            int[] returnVal = mr2.isInner(curVPos);

            if (returnVal != null)
            {
                Vector3 r = mr2.calcBarycentValue(returnVal, curVPos);

                Vector2 vp0 = new Vector2(dynamicPoints[returnVal[0]].x, dynamicPoints[returnVal[0]].y);
                Vector2 vp1 = new Vector2(dynamicPoints[returnVal[1]].x, dynamicPoints[returnVal[1]].y);
                Vector2 vp2 = new Vector2(dynamicPoints[returnVal[2]].x, dynamicPoints[returnVal[2]].y);
        ...
        newMesh.vertices = mv.ToArray();
        newMesh.normals = mn.ToArray();
        newMesh.triangles = mt.ToArray();
        newMesh.uv = muv.ToArray();
    ...
    public void generateDPS()
    {
        ...
        //float leftXValue = Vector3.Distance(dynamicPoints[0], dynamicPoints[36]) / 2.25f;
        //float rightValue = Vector3.Distance(dynamicPoints[16], dynamicPoints[45]) / 2.25f;

        //float leftXValue = Vector3.Distance(dynamicPoints[0], dynamicPoints[36]) / 2.0f;
        //float rightValue = Vector3.Distance(dynamicPoints[16], dynamicPoints[45]) / 2.0f;

        float leftXValue = Vector3.Distance(dynamicPoints[0], dynamicPoints[36]) / 1.0f;
        float rightValue = Vector3.Distance(dynamicPoints[16], dynamicPoints[45]) / 1.0f;
        ...
        //int[] neckLineIndex = { 3, 72, 69, 31, 33, 35, 68, 80, 13 };
        int[] neckLineIndex = { 4, 71, 48, 62, 54, 70, 12 };

        Vector3 neckLen = dynamicPoints[8] - dynamicPoints[29];
        for (int i = 0; i < neckLineIndex.Length; i++)
        {
            dynamicPoints.Add(dynamicPoints[neckLineIndex[i]] + neckLen);
        }
