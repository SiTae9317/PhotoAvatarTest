...
public class GradationGen : IDisposable
{
    Color[] sourceData;
    ...
    public float gradationValue = 1.0f;

    public GradationGen(Color[] source, float[] overlap)
    {
        sourceData = source;
        overlapData = overlap;
    ...
    void gradationOutline(List<int> outline, float value)
    {
        ...
        using (Dictionary<int, List<int>>.Enumerator graEnum = gradationIndexs.GetEnumerator())
        {
            while (graEnum.MoveNext())
            {
                int currentIndex = graEnum.Current.Key;
                overlapData[currentIndex] = value;

                List<int> aroundColors = graEnum.Current.Value;

                Color keepColor = sourceData[currentIndex];

                float keepR = keepColor.r;
                float keepG = keepColor.g;
                float keepB = keepColor.b;
                ...
                int aroundCount = aroundColors.Count;

                for (int i = 0; i < aroundCount; i++)
                {
                    int aroundCurrentIndex = aroundColors[i];

                    keepColor = sourceData[aroundCurrentIndex];
                    sumR += keepColor.r;
                    sumG += keepColor.g;
                    sumB += keepColor.b;
                }

                sumR /= (float)aroundCount;
                sumG /= (float)aroundCount;
                sumB /= (float)aroundCount;
                ...
                sumR = Mathf.Max(Mathf.Min(sumR, 1.0f), 0.0f);
                sumG = Mathf.Max(Mathf.Min(sumG, 1.0f), 0.0f);
                sumB = Mathf.Max(Mathf.Min(sumB, 1.0f), 0.0f);

                sourceData[currentIndex] = new Color(sumR, sumG, sumB);
            }
        }

        gradationIndexs.Clear();
        gradationIndexs = null;
    }
